    ...
    <>
      ...
      >
        <div className="container-main flex items-center justify-between h-16 md:h-[72px]">
          ...
          <motion.a
            href="#contact"
            className="hidden md:flex items-center gap-2 px-5 py-2 text-[13px] font-medium text-white rounded-full bg-primary/10 border border-primary/20 hover:bg-primary/20 hover:border-primary/40 transition-all duration-300"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            <motion.span
              className="block w-5 h-[1.5px] bg-text-primary rounded-full"
              animate={{
                rotate: isOpen ? 45 : 0,
                y: isOpen ? 3.25 : 0,
              }}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="fixed inset-0 z-40 bg-[#050816]/95 backdrop-blur-2xl flex flex-col items-center justify-center"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            ...
            <nav className="flex flex-col items-center gap-2">
              {NAV_LINKS.map((link, i) => (
                <motion.a
                  ...
                  className="text-2xl font-light text-text-secondary hover:text-text-primary transition-colors py-2"
